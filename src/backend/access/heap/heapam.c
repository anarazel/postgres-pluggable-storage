...
initscan(HeapScanDesc scan, ScanKey key, bool keep_startblock)
{
	...
	{
		bpscan = (ParallelBlockTableScanDesc) scan->rs_scan.rs_parallel;
		scan->rs_nblocks = bpscan->phs_nblocks;
	}
	else
		scan->rs_nblocks = RelationGetNumberOfBlocks(scan->rs_scan.rs_rd);
	...
	else if (allow_sync && synchronize_seqscans)
	{
		...
		scan->rs_startblock = ss_get_location(scan->rs_scan.rs_rd, scan->rs_nblocks);
	}
	else
	{
	...
	if (ScanDirectionIsForward(dir))
	{
		if (!scan->rs_inited)
		{
			...
			if (scan->rs_scan.rs_parallel != NULL)
			{
				...
				(ParallelBlockTableScanDesc) scan->rs_scan.rs_parallel;

				table_block_parallelscan_startblock_init(scan->rs_scan.rs_rd, pbscan);

				page = table_block_parallelscan_nextpage(scan->rs_scan.rs_rd, pbscan);
			...
			bool allow_strat, bool allow_sync, bool allow_pagemode)
{
	...
	if (scan->rs_scan.rs_parallel != NULL)
	{
		...
		table_block_parallelscan_reinitialize(sscan->rs_rd, parallel_scan);
	}
}
...
heap_getnext(TableScanDesc sscan, ScanDirection direction)
{
	...
	/*
	 * This is still widely used outside directly, without going through the
	 * table AM, so add a safety check.  It's possible we should at a later
	 * point downgrade this to an assert.
	 */
